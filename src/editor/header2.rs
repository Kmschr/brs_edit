...
pub fn ui(ui: &mut egui::Ui, header2: &mut Header2, save_colors: &mut Vec<([f32; 4], u32)>) {
    CollapsingHeader::new("Header2").default_open(true).show(ui, |ui| {
        ui.visuals_mut().override_text_color = None;
        if let Some(style) = ui.style_mut().text_styles.get_mut(&TextStyle::Button) {
            style.size = 14.0;
        }
        ui.add_space(5.0);
        list_ui(ui, "Mods", "No longer used, but can be found in older saves", "Add Mod", &mut header2.mods);
        ui.add_space(5.0);
        list_ui(
            ui,
            "Brick Assets",
...
fn list_ui(ui: &mut egui::Ui, header: &str, label: &str, button_label: &str, list: &mut Vec<String>) {
